...
type FingerSpellingTrackViewProps = {
  units: FsTrackUnit[];
};
...
export default function FingerSpellingTrackView({
  units,
}: FingerSpellingTrackViewProps) {
      ...
      <Stack spacing={1.5}>
        {units.map((unit) => (
          <UnitTrackCard
            ...
            onToggle={() => toggleUnitExpanded(unit.id)}
            locale={locale}
            unitLabel={t("fsUnit")}
    ...
    >
      <Stack spacing={2}>
        ...
        >
          ...
          >
            {locale === "kh"
              ? `បានបញ្ចប់ ${completedCount}/${totalCount} ${countLabel === "lessons" ? "មេរៀន" : "ជំពូក"}`
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            Current unit / {unit.completedLessonCount} of {unit.totalLessonCount}{" "}
            lessons completed
          </Typography>
          <Stack
            component="span"
...
function ChapterTrackSection({
  ...
  const expanded = useFingerSpellingStore((state) =>
    state.expandedChapterIds[chapter.id] === true
  );
