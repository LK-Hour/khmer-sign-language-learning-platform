...
import type { FsChapter, FsLesson, FsUnit } from "../types";
import {
  formatChapterBadge,
  formatUnitBadge,
} from "../utils/chapter";
...
const QUIZ_CARD = {
  ...
  description:
    "Exercise and quiz to track learner progress after all chapters.",
} as const;
...
export default function FingerSpellingTrack({ units }: FingerSpellingTrackProps) {
  ...
  const currentUnit =
    ...
    units[0];
  const currentLesson = currentUnit?.chapters
    .flatMap((chapter) => chapter.lessons)
    .find(
      (lesson) => lesson.progressStatus === "IN_PROGRESS" && !lesson.isLocked
    );
  ...
  return (
    <Stack spacing={{ xs: 2.5, md: 3 }} sx={{ maxWidth: 1280, mx: "auto" }}>
      <Stack
      ...
      >
        <Stack spacing={0.5}>
          ...
          >
            Learn Khmer characters step by step
          </Typography>
        </Stack>
        ...
        <Button
          ...
          href={
            currentLesson
              ? ROUTES.fingerSpelling.lesson(currentLesson.id)
              : ROUTES.fingerSpelling.root
          }
          variant="outlined"
          sx={{
      ...
      <Grid container spacing={2}>
        <Grid size={{ xs: 12, md: 6 }}>
          <TrackSummaryCard
            step={String(currentUnit?.orderIndex ?? 1).padStart(2, "0")}
            title={currentUnitTitle}
            description="Sample image, hand keypoints, live matching, and probability correction."
        ...
        <Grid size={{ xs: 12, md: 6 }}>
          <TrackSummaryCard
            ...
            description={QUIZ_CARD.description}
    ...
    >
      <Stack spacing={2}>
        ...
        >
          ...
          >
            {completedCount} of {totalCount} {countLabel} completed
          </Typography>
        </Stack>
    ...
    >
      ...
      >
        <Stack direction="row" spacing={2} sx={{ alignItems: "center", minWidth: 0 }}>
          <NumberBadge>{String(unit.orderIndex).padStart(2, "0")}</NumberBadge>
          <Typography
            sx={{
              color: KslColors.textPrimary,
              fontSize: { xs: KslFontSizes.md, md: KslFontSizes.lg },
        ...
        >
          ...
          >
            Current unit / {unit.completedLessonCount} of {unit.totalLessonCount} lessons completed
          </Typography>
          <Stack
            component="span"
            sx={{
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {chapter.lessonCount} lessons + practice
          </Typography>
          <Stack
            component="span"
            sx={{
...
function LessonTrackRow({
  ...
  const title = `Lesson ${String(lesson.orderIndex).padStart(2, "0")}`;
  const subtitle =
    locale === "kh"
      ? lesson.letterNameKh || lesson.letterNameEn || lesson.letter
      : lesson.letterNameEn || lesson.letterNameKh || lesson.letter;
    ...
    >
      ...
      >
        {state !== "lock" && (
          <Typography
            sx={{
              color: state === "done" ? KslColors.success : KslColors.inProgress,
              fontSize: KslFontSizes.sm,
              fontWeight: 600,
            }}
          >
  ...
  return (
    <Link href={ROUTES.fingerSpelling.lesson(lesson.id)} style={{ color: "inherit", textDecoration: "none" }}>
      {row}
    </Link>
  );
}
