 ...
 */
export function useSentenceRealtimePredictor() {
  ...
  const connect = useCallback(() => {
    ...
    const ws = new WebSocket(`${baseUrl}/api/sentence_spelling/ws/predict`);
    wsRef.current = ws;
  ...
  const sendFeatures = useCallback(
    (features: number[], handedness?: string, targetLabel?: string) => {
      const ws = wsRef.current;
