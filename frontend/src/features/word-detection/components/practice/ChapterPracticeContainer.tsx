...
import { fetchWdChapterPractice } from "../../api/curriculum";
...
export default function ChapterPracticeContainer({
  ...
  const units = useWordDetectionStore((state) => state.units);
  ...
  const [practice, setPractice] = useState<WdChapterPractice | null>(null);
  ...
  useEffect(() => {
    ...
    void fetchWdChapterPractice(chapterId)
      .then((data) => {
        if (ignore) return;
        if (!data) {
          setFetchState("error");
  ...
  if (!isUnlocked) {
    return (
      <Stack spacing={2} sx={{ alignItems: "center", py: 8, textAlign: "center" }}>
        ...
        >
          {t("WORD_DETECTION.PRACTICE.LOCKED_TITLE")}
        </Typography>
        <Typography sx={{ color: KslColors.textSecondary, fontSize: KslFontSizes.md }}>
          {t("WORD_DETECTION.PRACTICE.LOCKED_HINT")}
        </Typography>
        <Button
          component={Link}
        ...
        >
          {t("WORD_DETECTION.PRACTICE.BACK_TO_TRACK")}
        </Button>
      </Stack>
    );
  }
