...
function NumberBadgeSkeleton() {
  return <Skeleton variant="rounded" width={42} height={42} sx={{ borderRadius: 2.5 }} />;
}
...
function SummaryCardSkeleton() {
  return (
    <Paper
      elevation={0}
      sx={{ height: "100%", border: `1px solid ${KslColors.border}`, borderRadius: `${KslRadii.card}px`, p: { xs: 2.25, md: 3 } }}
    >
      <Stack spacing={2}>
...
function UnitCardSkeleton({ expanded = false }: { expanded?: boolean }) {
  return (
    <Paper
      elevation={0}
      sx={{ border: `1px solid ${KslColors.border}`, borderRadius: `${KslRadii.card}px`, overflow: "hidden" }}
    >
      <Stack
        direction="row"
        spacing={2}
        sx={{ alignItems: "center", justifyContent: "space-between", p: { xs: 1.5, md: 2 } }}
      >
        ...
        <Stack direction="row" spacing={1.5} sx={{ alignItems: "center", flexShrink: 0 }}>
          <Skeleton width={220} height={22} sx={{ display: { xs: "none", md: "block" } }} />
          <Skeleton variant="circular" width={34} height={34} />
        </Stack>
      </Stack>
      ...
      {expanded ? (
        <Stack spacing={1.5} sx={{ borderTop: `1px solid ${KslColors.border}`, p: 2 }}>
          <Paper
            elevation={0}
            sx={{ border: `1px solid ${KslColors.border}`, borderRadius: `${KslRadii.wordCard + 6}px`, overflow: "hidden" }}
          >
            <Stack
              direction="row"
              sx={{ alignItems: "center", justifyContent: "space-between", p: { xs: 1.5, md: 2 } }}
...
export function WordDetectionTrackSkeleton({ embedded = false }: { embedded?: boolean }) {
  const content = (
...
export default function WordDetectionPageLoading() {
  return (
    <Stack
      aria-busy
      aria-label="Loading word detection"
      sx={{ position: "fixed", inset: 0, zIndex: 1200, bgcolor: "background.default", overflowY: "auto" }}
    >
      <MainHeaderSkeleton />
      <PageContainer sx={{ py: { xs: 2.5, md: 4 } }}>
        <WordDetectionTrackSkeleton embedded />
      </PageContainer>
    </Stack>
  );
}
