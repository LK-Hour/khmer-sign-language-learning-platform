"""Business logic for word detection curriculum reads and admin CRUD operations."""
...
class WordDetectionCurriculumService:
    """Learner-facing curriculum queries + admin CRUD operations for Word Detection."""

    def __init__(self, db: Session) -> None:
        self.db = db
        self.curriculum = WordDetectionCurriculumRepository(db)
        self.progress = WordDetectionProgressRepository(db)
        self._exercise_repo = WordDetectionExerciseRepository(db)
        # Admin CRUD repositories — delegate to BaseCrudRepository via track config
        self.cfg = get_track_config("word_detection")
    ...
    def _unit_response(self, unit: WordDetectionUnit) -> UnitResponse:
        data = UnitResponse.model_validate(unit)
        data.chapter_count = self.chapters.count(unit_id=unit.id)
        return data

    def _chapter_response(self, chapter: WordDetectionChapter) -> WdChapterAdminResponse:
        data = WdChapterAdminResponse.model_validate(chapter)
        data.lesson_count = self.lessons.count(chapter_id=chapter.id)
        data.exercise_count = self._exercise_repo.count_by_chapter(chapter.id)
        return data

    def _lesson_response(self, lesson: WordDetectionLesson) -> LessonResponse:
        data = LessonResponse.model_validate(lesson)
        data.exercise_count = self._exercise_repo.count_by_lesson(lesson.id)
        return data
    ...
    def admin_get_chapter(self, chapter_id: int) -> WdChapterAdminResponse | None:
        chapter = self.chapters.get(chapter_id)
        return self._chapter_response(chapter) if chapter else None

    def admin_create_chapter(self, body: WdChapterCreate) -> WdChapterAdminResponse | None:
    ...
    def admin_update_chapter(
        self, chapter_id: int, body: WdChapterUpdate
    ) -> WdChapterAdminResponse | None:
        chapter = self.chapters.get(chapter_id)
        if chapter is None:
            return None
    ...
    def admin_delete_chapter(self, chapter_id: int) -> WdChapterAdminResponse | None:
        chapter = self.chapters.get(chapter_id)
        if chapter is None:
            return None
