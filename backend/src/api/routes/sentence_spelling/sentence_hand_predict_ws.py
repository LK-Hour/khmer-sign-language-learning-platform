"""WebSocket endpoint for real-time sentence-spelling hand prediction (streaming).

Mirrors ``finger_spelling/finger_hand_predict_ws.py``'s protocol exactly
(``auth`` / ``predict`` / ``predict_label_match``) against the sentence-
spelling model instead.
"""
...
from src.services.sentence_spelling.sentence_hand_prediction_service_label_match import (
...
async def handle_websocket(websocket: WebSocket) -> None:
    ...
    service = get_sentence_hand_prediction_service()
    if not service.is_available:
        await websocket.send_json(
            WsErrorResponse(message="Sentence hand prediction model is not configured").model_dump()
        )
        await websocket.close(code=status.WS_1011_INTERNAL_ERROR)
        return
    label_match_service = get_sentence_hand_label_match_prediction_service()
    ...
    try:
        ...
        logger.info("Sentence-spelling WebSocket disconnected (subject=%s)", subject_id)
    except Exception:
        logger.exception("Unexpected sentence-spelling WebSocket error")
