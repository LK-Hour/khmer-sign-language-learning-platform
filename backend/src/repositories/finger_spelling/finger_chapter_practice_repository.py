"""Data access for finger spelling chapter-level practice sessions."""
...
class FingerChapterPracticeRepository:
    def __init__(self, db: Session) -> None:
        self.db = db

    # ── FingerPractice rows (1:1 with chapter) ──────────────────────────────

    def get_practice_by_chapter(self, chapter_id: int) -> FingerPractice | None:
        stmt = select(FingerPractice).where(FingerPractice.chapter_id == chapter_id)
        return self.db.scalars(stmt).first()

    def get_or_create_practice(self, chapter_id: int, lesson_count: int) -> FingerPractice:
        practice = self.get_practice_by_chapter(chapter_id)
        if practice is not None:
            return practice
        practice = FingerPractice(
    ...
    # ── FingerUserPracticeProgress rows ──────────────────────────────────────

    def get_user_progress(
        self, user_id: uuid.UUID, practice_id: int
    ) -> FingerUserPracticeProgress | None:
        stmt = select(FingerUserPracticeProgress).where(
            FingerUserPracticeProgress.user_id == user_id,
            FingerUserPracticeProgress.finger_practice_id == practice_id,
        )
        return self.db.scalars(stmt).first()
    ...
    def upsert_user_progress(
        ...
        *,
        ...
        if progress is None:
            progress = FingerUserPracticeProgress(
                user_id=user_id,
                finger_practice_id=practice_id,
